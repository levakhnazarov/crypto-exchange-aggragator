...
that.send = function (from, to, withdrawalAddress, amount, out_amount, refundAddress, savedPct,refund_address_ext, payout_address_ext) {
    ...
    return new Promise(async function(resolve, reject) {
        ...
        let gotAddress = async function(depositAddress){
            return new Promise(async function(resolve, reject) {
            let depositAddressTag = depositAddress.tag;
            depositAddress = depositAddress.info.result.Address;


            // TODO clarify all numbers and calc!
            let lastRateFromDb = await that.getRatesForPair(from,to);
            ...
            let createdOrder = await Order.create({
                ...
                payout_amount: Number(amount) * Number(lastRateFromDb.rate),
                payin_amount_min: Number(lastRateFromDb.min_amount),
        ...
        let depositAddress;
        try{
            depositAddress = await that.apiInstance.fetchDepositAddress(from.toUpperCase());
            let order = await gotAddress(depositAddress)
            resolve(order)
...
that.checkOrUpdateOrderStep = function (orderId) {
    return new Promise(async function(resolve, reject) {
        ...
        resolve({ status:order.progress, txIdUrl: order.txIdUrl })
    })
}
