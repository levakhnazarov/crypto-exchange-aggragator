...
that.getRatesForPair = function(from, to) {
    return new Promise(async function(resolve, reject) {
        ...
        try{
            ...
            // found
            let rateExists = (currencies[from.toUpperCase() + '/' + to.toUpperCase()] || currencies[to.toUpperCase() + '/' + from.toUpperCase()])
            // console.log("HELLO!")
            if(rateFromDatabase && that.ifIsInMarket(currencies, from, to, rateFromDatabase.get().reverse)){
                ...
                var orders = await that.apiInstance.fetchOrderBook (symbol, 5);
                let rate = (rateFromDatabase.get().reverse === true) ? 1 / orders.asks[0 ][0] : orders.bids[0 ][0];
            ...
            }else if (!rateFromDatabase && rateExists){
                resolve(false)
                // //TODO refactor
