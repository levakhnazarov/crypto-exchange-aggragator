...
var  fetchRates = async function () {
    ...
    let exchange = new ccxt.kucoin({
        'apiKey': models.conf.api.kucoin.apiKey,
        'secret': models.conf.api.kucoin.secret,
        // 'verbose':true,
        'timeout': 60000,
    ...
    let ratesFromDatabase = await Rate.findAll({where: {exchanger: "kucoin"}})

    console.log(ratesFromDatabase.length + ' - rates from database length')

    var additionalRatesToCreate = allKucoinPairs.filter(item => {
    ...
    var ratesToRemove = ratesFromDatabase.filter(model => {
        return allKucoinPairs.filter(item => {
            return model.currency_from.toLowerCase() === item.from.toLowerCase() &&
                model.currency_to.toLowerCase() === item.to.toLowerCase()
        }).length === 0
    });
    ...
    for (let i = 0; i < ratesFromDatabase.length; i++) {
        ...
        let item = allKucoinPairs.reduce((find, item) => {
