const shapeshiftInstance = require('../exchangers').components.binance;
...
var  fetchRates = async function () {
    ...
    let exchange = new ccxt.poloniex({
        'apiKey': models.conf.api.poloniex.apiKey,
        'secret': models.conf.api.poloniex.secret,
        // 'verbose':true,
        'timeout': 60000,
    ...
    let ratesFromDatabase = await Rate.findAll({where: {exchanger: "poloniex"}})

    console.log(ratesFromDatabase.length + ' - rates from database length')

    var additionalRatesToCreate = allPoloniexPairs.filter(item => {
    ...
    var ratesToRemove = ratesFromDatabase.filter(model => {
        return allPoloniexPairs.filter(item => {
            return model.currency_from.toLowerCase() === item.from.toLowerCase() &&
                model.currency_to.toLowerCase() === item.to.toLowerCase()
        }).length === 0
    });
    ...
    for (let i = 0; i < ratesFromDatabase.length; i++) {
        ...
        let item = allPoloniexPairs.reduce((find, item) => {
