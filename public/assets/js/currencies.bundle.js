...
var _CurrenciesTable = __webpack_require__(95);

var _CurrenciesTable2 = _interopRequireDefault(_CurrenciesTable);
...
var Currencies = function () {
  function Currencies() {
    ...
    this.$table = new _CurrenciesTable2.default(document.querySelector('[data-currencies-table]'));

    this.$table.on('sort', function (sort) {
      return _this._fetch(false);
    });
  ...
  _createClass(Currencies, [{
    ...
    value: function _initEvents() {
      var _fetch = (0, _debounce2.default)(this._update.bind(this), 250);
      this.$searchField.addEventListener('input', _fetch);

      var _more = (0, _throttle2.default)(this._more.bind(this), 500);
    ...
    value: function _fetch() {
      ...
      fetch(window.location.pathname, {
        ...
        if (more) {
          _this2.$table.append(modifiedRows);
        } else {
          _this2.$table.update(modifiedRows);
        }
        _this2.$totalCount = modifiedRows.length;
        _this2._hidePreloader();
      }).catch(function (err) {
    ...
    key: '_visiblePreloader',
    value: function _visiblePreloader() {
...
var _currencyFormatter = __webpack_require__(96);
...
var CurrenciesTable = function (_EventEmitter) {
  _inherits(CurrenciesTable, _EventEmitter);

  function CurrenciesTable($el) {
    _classCallCheck(this, CurrenciesTable);

    var _this = _possibleConstructorReturn(this, (CurrenciesTable.__proto__ || Object.getPrototypeOf(CurrenciesTable)).call(this));

    _this.$table = $el;
    _this.$exchangeBtnTpl = $el.getAttribute('data-currencies-exchange-btn-template') || 'Exchange {currency}';
    _this.$thead = $el.querySelector('thead');
    _this.$tbody = $el.querySelector('tbody');
    _this._updateTotal();

    _this.$thead.addEventListener('click', function (e) {
      var el = e.target.closest('[data-sort-col]');

      if (el && el.dataset && el.dataset.sortCol) {
        var current = _this._getCurrentSortEl();
        if (current && current !== el) {
          current.dataset.sortDirection = '';
        }
        el.dataset.sortDirection = el.dataset.sortDirection == 'desc' ? 'asc' : 'desc';
        _this.emit('sort', _this.getCurrentSort());
      }
    });
    return _this;
  }

  _createClass(CurrenciesTable, [{
    key: '_getCurrentSortEl',
    value: function _getCurrentSortEl() {
      return this.$thead.querySelector('th:not([data-sort-direction=""])');
    }
  }, {
    key: 'getCurrentSort',
    value: function getCurrentSort() {
    ...
    key: '_renderTable',
    value: function _renderTable(data, append) {
      var _this2 = this;
      ...
      var _loop = function _loop() {
        ...
        var el = _this2._createRow(data[i]);
    ...
    key: '_createRow',
    value: function _createRow(data) {

      var prettyFormattedPrice = _currencyFormatter2.default.format(data.price_usd, {
        decimal: ',',
        thousand: ' ',
        precision: 2
      });
    ...
    key: '_createChangesColumn',
    value: function _createChangesColumn(className, pct, usd) {
...
var accounting = __webpack_require__(97);
var assign = __webpack_require__(98);
var localeCurrency = __webpack_require__(99);
var currencies = __webpack_require__(101);
var localeFormats = __webpack_require__(102);
